...
for i in range(roi_start, roi_end, 1):
    ...
    I_mean = np.uint8(np.mean(BUF, axis=2))
    I_mean_diff = cv2.absdiff(I_mean, IG_t2)
    ...
    dilation_mean = cv2.dilate(erosion_mean , kernel_mean , iterations=1)
    ...
    I_median = np.uint8(np.median(BUF, axis=2))
    I_median_diff = cv2.absdiff(I_median, IG_t2)
    ...
    dilation_median = cv2.dilate(erosion_median, kernel_median, iterations=1)
