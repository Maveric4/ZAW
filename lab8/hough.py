...
contours, hierarchy = cv2.findContours(im_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    for y in range(0, h):
        if gradient_value2[x, y] > 0.5:
            ...
            if phi == 360:
                phi = 0
            for i in Rtable[phi]:
                x1 = int(round(i[0] * np.cos(i[1]*np.pi/180) + x))
                y1 = int(round(i[0] * np.sin(i[1]*np.pi/180) + y))
                result[y1, x1] += 1
...
cv2.circle(im2_color, found_mass_center, 1, (0, 255, 0), thickness=3)

for j in range(0, 15):
    result[found_mass_center] = 0
    found_mass_center = np.unravel_index(result.argmax(), result.shape)
    cv2.circle(im2_color, found_mass_center, 1, (0, 255, 0), thickness=3)
